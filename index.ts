...
class Bank {
  ...
  addAccountNum(obj: Bank_Account) {
    this.account.push(obj);
  }
  Transaction(acctObj: Bank_Account) {
    let new_accounts = this.account.filter(
      (acct) => acct.account_No !== acctObj.account_No
    );
...
async function Bank_Services(bank: Bank) {
  do {
    ...
    if (services.Select_service == "View_Balance") {
      ...
      let account = My_Bank.account.find(
        (Acc) => Acc.account_No == view_res.num
      );
      if (!account) {
        console.log(chalk.red.bold("Invalid Account Number!!!"));
      }
      if (account) {
        let name = My_Bank.Customer.find(
          (item) => item.account_No == account?.account_No
        );
        console.log(
    ...
    if (services.Select_service == "Cash_Withdrawal") {
      ...
      if (account) {
        ...
        if (Ans.Rupees > account.Balance) {
          console.log(
            chalk.bold.redBright.italic("Current Balance is insufficient")
          );
        }
        let New_Balance = account.Balance - Ans.Rupees;
        //trasaction method call
